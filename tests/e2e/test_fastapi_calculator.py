...
def test_create_calculation_absolute_difference(base_url: str):
    user_data = {
        "first_name": "Calc",
        "last_name": "AbsDiff",
        "email": f"calc.absdiff{uuid4()}@example.com",
        "username": f"calc_absdiff_{uuid4()}",
        "password": "SecurePass123!",
        "confirm_password": "SecurePass123!"
    }
    token_data = register_and_login(base_url, user_data)
    ...
    data = response.json()
    # Expected result: |10 - (-3)| = |10 + 3| = 13
    assert "result" in data and data["result"] == 13, f"Expected result 13, got {data.get('result')}"
...
def test_model_power():
    ...
    result = calc.get_result()
    assert result == 8, f"Power result incorrect: expected 8, got {result}"

def test_model_root():
    dummy_user_id = uuid4()
    calc = Calculation.create("root", dummy_user_id, [100, 2])
    result = calc.get_result()
    assert result == 10, f"Root result incorrect: expected 10, got {result}"

    with pytest.raises(ValueError):
        calc_zero = Calculation.create("root", dummy_user_id, [100, 0])
        calc_zero.get_result()

    # Negative even root returns complex number, not ValueError
    calc_negative = Calculation.create("root", dummy_user_id, [-100, 2])
    result_negative = calc_negative.get_result()
    # Python returns a complex number for this, verify it works without error
    assert result_negative is not None

def test_model_absolute_difference():
    dummy_user_id = uuid4()
    calc = Calculation.create("absolute_difference", dummy_user_id, [10, -3])
    result = calc.get_result()
    assert result == 13, f"Absolute difference result incorrect: expected 13, got {result}"