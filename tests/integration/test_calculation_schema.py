...
def test_calculation_base_division_by_zero_validation():
    """Test that division schema prevents zero divisor"""
    data = {
        "type": "division",
        "inputs": [100, 0, 5]
    }
...
def test_calculation_base_non_division_with_zero():
    ...
    calc = CalculationBase(**data)
    assert calc.inputs == [5, 0, 3]

def test_calculation_base_single_input():
    """Test that single input fails validation"""
    data = {"type": "addition", "inputs": [5]}
    with pytest.raises(ValidationError):
        CalculationBase(**data)

def test_calculation_create_valid():
    """Test valid CalculationCreate"""
    user_id = uuid4()
    data = {
        "type": "addition",
        "inputs": [1, 2, 3],
...
def test_calculation_create_invalid_userid():
    """Test CalculationCreate with invalid userid"""
    data = {
        "type": "addition",
        "inputs": [1, 2, 3],
        "user_id": "not-a-uuid"
...
def test_calculation_update_valid():
    ...
    assert update.inputs == [10, 5]

def test_calculation_update_none():
    """Test CalculationUpdate with no inputs (partial update)"""
    data = {}
    update = CalculationUpdate(**data)
    assert update.inputs is None

def test_calculation_update_insufficient_inputs():
    """Test CalculationUpdate with fewer than 2 inputs"""
    data = {"inputs": [5]}
    with pytest.raises(ValidationError):
        CalculationUpdate(**data)
